                ...
                <div class="card shadow h-100">
                    <div class="card-header border-0">
                        <div class="d-flex flex-column flex-md-row align-items-center justify-content-center justify-content-md-between text-center text-md-left">
                            <div class="mb-3">
                                <h2 class="mb-0">Edit Periode</h2>
                                <p class="mb-0 text-sm">Kelola Analisis - {{ $periode->analisis->nama }}</p>
                            </div>
                            <div class="mb-3">
                                <a href="{{ route('periode.index', $periode->analisis_id) }}?page={{ request('page') }}" class="btn btn-success" title="Kembali"><i class="fas fa-arrow-left"></i> Kembali</a>
                            </div>
                        </div>
                    </div>
                </div>
...
<div class="card bg-secondary shadow h-100">
    <div class="card-body">
        <form autocomplete="off" action="{{ route('periode.update', [$analisis,$periode]) }}" method="post">
            @csrf @method('patch')
            <div class="form-group row">
                <label class="form-control-label col-form-label col-md-3" for="nama">Nama Periode</label>
                <div class="col-md-9">
                    <input type="text" class="form-control @error('nama') is-invalid @enderror" name="nama" placeholder="Masukkan Nama Periode ..." value="{{ old('nama', $periode->nama) }}">
            ...
            <div class="form-group row">
                ...
                <div class="col-md-9">
                    <select class="form-control @error('tahap_pendataan') is-invalid @enderror" name="tahap_pendataan">
                        ...
                        <option value="1" {{ old('tahap_pendataan', $periode->tahap_pendataan) == 1 ? 'selected' : '' }}>Belum Pendataan</option>
                        <option value="2" {{ old('tahap_pendataan', $periode->tahap_pendataan) == 2 ? 'selected' : '' }}>Sedang Pendataan</option>
                        <option value="3" {{ old('tahap_pendataan', $periode->tahap_pendataan) == 3 ? 'selected' : '' }}>Selesai Pelaksanaan</option>
                    </select>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-9">
                    <input type="number" class="form-control @error('tahun') is-invalid @enderror" name="tahun" placeholder="Masukkan Tahun ..." value="{{ old('tahun', $periode->tahun) }}">
            ...
            <div class="form-group row">
                ...
                <div class="col-md-9">
                    <textarea class="form-control @error('keterangan') is-invalid @enderror" name="keterangan" placeholder="Masukkan Keterangan ...">{{ old('keterangan', $periode->keterangan) }}</textarea>
