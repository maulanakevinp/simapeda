...
<body onload="window.print()" oncontextmenu="return false;">
    <div class="text-center mb-3" style="margin-top:20px">
        <h5 class="font-weight-bold text-uppercase">Desa {{ $desa->nama_desa }} Kecamatan {{ $desa->nama_kecamatan }} Kabupaten {{ $desa->nama_kabupaten }}<br>Agenda Surat Masuk {{ $tahun ? "Tahun $tahun" : '' }}</h5>
    </div>
        ...
        <tbody>
            @foreach ($surat_masuk as $item)
                <tr>
                    ...
                    <td class="text-center" width="120px">{{ tgl($item->tanggal_penerimaan) }}</td>
